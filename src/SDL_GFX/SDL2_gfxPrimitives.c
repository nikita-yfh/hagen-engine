...
int filledRectangleColor(SDL_Renderer * renderer, Sint16 x1, Sint16 y1, Sint16 x2, Sint16 y2, Uint32 color) {
	Uint8 *c = (Uint8 *)&color;
	return filledRectangleRGBA(renderer, x1, y1, x2, y2, c[0], c[1], c[2], c[3]);
}
int filledRectangleRGBA(SDL_Renderer * renderer, Sint16 x1, Sint16 y1, Sint16 x2, Sint16 y2, Uint8 r, Uint8 g, Uint8 b, Uint8 a) {
	int result;
	Sint16 tmp;
	SDL_Rect rect;
	if (x1 == x2) {
...
int _HLineTexturedSur(SDL_Surface *dst, Sint16 x1, Sint16 x2, Sint16 y, SDL_Surface *texture, int texture_w, int texture_h, int texture_dx, int texture_dy) {
	Sint16 w;
	Sint16 xtmp;
	int result = 0;
	int texture_x_walker;
	...
	*/
	...
	for (y = miny; (y <= maxy); y++) {
		...
		for (i = 0; (i < ints); i += 2) {
			...
			xb = gfxPrimitivesPolyInts[i+1] - 1;
			xb = (xb >> 16) + ((xb & 32768) >> 15);
			result |= _HLineTextured(renderer, xa, xb, y, tex, texture->w, texture->h, texture_dx, texture_dy);
		}
	...
	*/
	...
	for (y = miny; (y <= maxy); y++) {
		...
		for (i = 0; (i < ints); i += 2) {
			...
			xb = gfxPrimitivesPolyInts[i+1] - 1;
			xb = (xb >> 16) + ((xb & 32768) >> 15);
			result |= _HLineTexturedSur(dst, xa, xb, y, texture, texture->w, texture->h, texture_dx, texture_dy);
		}
	}
	...
	*/
	...
	for (y = miny; (y <= maxy); y++) {
		...
		for (i = 0; (i < ints); i += 2) {
			...
			xb = gfxPrimitivesPolyInts[i+1] - 1;
			xb = (xb >> 16) + ((xb & 32768) >> 15);
			result |= _HLineTextured(renderer, xa, xb, y, texture, w, h, texture_dx, texture_dy);
		}
	}
...
int texturedPolygonSur(SDL_Surface *dst, const Sint16 * vx, const Sint16 * vy, int n, SDL_Surface *texture, int texture_dx, int texture_dy) {
	/*
	* Draw
	*/
	return (texturedPolygonMTSur(dst, vx, vy, n, texture, texture_dx, texture_dy, NULL, NULL));
}

int texturedPolygonTex(SDL_Renderer *dst, const Sint16 * vx, const Sint16 * vy, int n, SDL_Texture *texture, int texture_dx, int texture_dy) {
	/*
	* Draw
	*/
	return (texturedPolygonMTTex(dst, vx, vy, n, texture, texture_dx, texture_dy, NULL, NULL));
}
